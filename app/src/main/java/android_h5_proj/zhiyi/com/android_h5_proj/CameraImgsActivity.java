 ...
 */
public class CameraImgsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        locationWebView.loadUrl("file:///android_asset/page_camera_imgs.html");
    }
    ...
    private void takePhoto() {
        try {
            ...
            File file = new File(dirFile, "camera_0" + (index++) + ".jpg");
            if (!file.exists()) {
                try {
                    file.createNewFile();
                } catch (Exception e) {
    ...
    public void setCropImg(final String tempDir) {
        ...
        if (tempDir != null) {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    locationWebView.loadUrl("javascript:display_imgs('" + tempDir + "')");
                }
            });
        }
    }
